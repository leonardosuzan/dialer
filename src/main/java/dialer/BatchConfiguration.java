...
public class BatchConfiguration {
    ...
    public ItemReader<Contato> reader() {
        ...
        reader.setLineMapper(new DefaultLineMapper<Contato>() {{
            setLineTokenizer(new DelimitedLineTokenizer() {{
                setNames(new String[] { "primeiroNome","sobrenome","codigoArea","numeroTelefonico","endereco","numeroEndereco","complemento","bairro","cidade","estado","observacoes" });
            }});
    ...
    // tag::jobstep[]
    @Bean
    public Job importContactsJob(JobBuilderFactory jobs, Step s1, JobExecutionListener listener) {
    ...
    @Bean
    public Step step1(StepBuilderFactory stepBuilderFactory, ItemReader<Contato> reader,
            ItemWriter<Contato> writer, ItemProcessor<Contato, Contato> processor) {
        return stepBuilderFactory.get("step1")
                .<Contato, Contato> chunk(10)
                .reader(reader)
                .processor(processor)
                .writer(writer)
                .build();
    ...
    @RequestMapping("testeImport")
    public void handle() throws Exception{
        jobLauncher.run(importContactsJob, new JobParameters());
    }
